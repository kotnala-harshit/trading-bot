...
The public GitHub Pages view uses a responsive portfolio layout with account KPIs, exposure and drawdown controls, holdings, recent fills, schedule health, and a portfolio-versus-Nifty chart built from successful scans. One-click tabs separate active Indian paper results from locked Phase 2 US and Phase 3 global research. Phase 2 uses community point-in-time S&P 500 membership, adjusted Yahoo prices, adjusted SPY, and USD/INR attribution. It remains locked because only 81.5% of requested historical members passed free-price checks, the five-year baseline drawdown reached 33.21%, and dividend withholding could not be isolated from adjusted prices. A defensive diagnostic reduced drawdown to roughly 10% but did not sustain a 70–80% win rate or competitive long-term returns. Later phases cannot create simulated orders until they pass their own validation. The automation is enabled by [`configs/paper-trader.json`](configs/paper-trader.json). To stop new paper activity from GitHub, change `"enabled": true` to `false` and commit it on `main`. Existing positions remain recorded but no simulated orders are created while disabled.
