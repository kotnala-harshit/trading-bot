...
The public GitHub Pages view uses a responsive portfolio layout with account KPIs, exposure and drawdown controls, holdings, recent fills, schedule health, and a portfolio-versus-Nifty chart built from successful scans. One-click tabs separate Indian and US paper observation from locked Phase 3 global research. Phase 2 uses a separate $10,000 simulated ledger, community S&P 500 membership, adjusted Yahoo prices, adjusted SPY, a 7% volatility target, a 40% exposure ceiling, and a SPY 200-day EMA cash gate. This is observation only: the historical model still failed its promotion gates and no broker or real-order path exists. The automation is controlled by [`configs/paper-trader.json`](configs/paper-trader.json); set `"us_enabled": false` to stop new US paper activity.
