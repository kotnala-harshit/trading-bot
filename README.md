...
The public GitHub Pages view uses a responsive portfolio layout with account KPIs, exposure and drawdown controls, holdings, recent fills, schedule health, and a portfolio-versus-Nifty chart built from successful scans. One-click tabs separate active Indian paper results from locked Phase 2 US and Phase 3 global research. Phase 2 now displays its ten-year study with an untouched five-year holdout, but remains locked because a fixed current shortlist is not survivorship-free and its holdout only matched adjusted SPY. Later phases cannot create simulated orders until they pass their own validation. The automation is enabled by [`configs/paper-trader.json`](configs/paper-trader.json). To stop new paper activity from GitHub, change `"enabled": true` to `false` and commit it on `main`. Existing positions remain recorded but no simulated orders are created while disabled.
